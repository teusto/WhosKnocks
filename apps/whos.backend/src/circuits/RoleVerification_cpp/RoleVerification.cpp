...
void RoleVerification_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void RoleVerification_1_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[2] = { 
IsZero_0_run,
RoleVerification_1_run };
...
ctx->componentMemory[coffset].templateName = "RoleVerification";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
RoleVerification_1_create(1,0,ctx,"main",0);
RoleVerification_1_run(0,ctx);
}
